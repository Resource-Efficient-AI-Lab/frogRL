...
template <typename T>
std::vector<float> flatten_vector(const std::vector<T>& input)
{
    ...
    else
    {
        ...
        for (const auto& element : input)
        {
            ...
            output.insert(output.end(), sub_vector.begin(), sub_vector.end());
        }
        return output;
    }
}
...
int main(int argc, char *argv[])
{
    try
    {
        ...
        // Prepare observation shape and calculate expected size
        auto observation_shape = env_info->observation_space_shape;
        observation_shape.insert(observation_shape.begin(), num_envs);

        size_t expected_size = num_envs;
        for (const auto &dim : env_info->observation_space_shape) {
            expected_size *= dim;
        }

        torch::Tensor observation;
        
        // Get observation data based on space type
        std::vector<float> observation_vec;
        if (env_info->observation_space_shape.size() > 1) {
            auto response = communicator.get_response<CnnResetResponse>();
            spdlog::debug("Raw CNN reset response: {}", response->observation);
            observation_vec = flatten_vector(response->observation);
        } else {
            auto response = communicator.get_response<MlpResetResponse>();
            spdlog::debug("Raw MLP reset response: {}", response->observation);
            observation_vec = flatten_vector(response->observation); 
        }

        // Verify dimensions match and create tensor
        if (observation_vec.size() != expected_size) {
            throw std::runtime_error(fmt::format(
                "Observation size mismatch: expected {}, got {}", 
                expected_size, observation_vec.size()));
        }

        // Create tensor with verified dimensions 
        observation = torch::from_blob(observation_vec.data(), 
            observation_shape).clone().to(device);
        ...
        Policy policy(space, base, env_info->observation_space_shape.size() == 1);
        policy->to(device);
        ...
        std::vector<float> running_rewards(num_envs, 0.0f);
        int episode_count = 0;
        bool render = false;
        std::vector<float> reward_history(reward_average_window_size, 0.0f);
        ...
        for (int update = 0; update < num_updates; ++update)
        {
            for (int step = 0; step < batch_size; ++step)
            {
                ...
                {
                    actions[i].reserve(env_info->action_space_shape[0]);
                ...
                if (env_info->observation_space_shape.size() > 1)
                {
                    auto cnn_step_result = communicator.get_response<CnnStepResponse>();
                    if (!cnn_step_result) {
                        throw std::runtime_error("Failed to get CNN step response");
                    }
                    observation_vec = flatten_vector(cnn_step_result->observation);
                    real_rewards = flatten_vector(cnn_step_result->real_reward);
                    dones_vec = cnn_step_result->done;
                }
                else
                {
                    auto mlp_step_result = communicator.get_response<MlpStepResponse>();
                    if (!mlp_step_result) {
                        throw std::runtime_error("Failed to get MLP step response");
                    }
                    observation_vec = flatten_vector(mlp_step_result->observation);
                    real_rewards = flatten_vector(mlp_step_result->real_reward);
                    dones_vec = mlp_step_result->done;
                }

                // Verify observation size before creating tensor
                if (observation_vec.size() != expected_size) {
                    throw std::runtime_error(fmt::format(
                        "Step observation size mismatch: expected {}, got {}", 
                        expected_size, observation_vec.size()));
                }
                ...
                auto reward_tensor = torch::from_blob(real_rewards.data(), 
                    {num_envs}, torch::kFloat);
                returns = returns * discount_factor + reward_tensor;
                returns_rms->update(returns);

                reward_tensor = torch::clamp(
                    reward_tensor / torch::sqrt(returns_rms->get_variance() + 1e-8),
                    -reward_clip_value, reward_clip_value);
                
                rewards = std::vector<float>(
                    reward_tensor.data_ptr<float>(),
            ...
            float decay_level = use_lr_decay ? 
                1.0f - static_cast<float>(update) / num_updates : 
                1.0f;
            ...
            if (update % log_interval == 0 && update > 0)
            {
                ...
                float average_reward = std::accumulate(
                    reward_history.begin(), 
                    reward_history.end(), 
                    0.0f);
                average_reward /= episode_count < reward_average_window_size ? 
                    episode_count : reward_average_window_size;
