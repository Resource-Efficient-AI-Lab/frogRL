...
# from stable_baselines3.common.env_util import make_atari_env, wrap_deepmind
...
#     def reset(self):
#         print("Resetting the environment...")  # Debug print
#         obs = self.venv.reset()  # Assuming this is where the reset happens
#         print("Observations received from environment:", obs)  # Print the observations
#         return obs
...
#                 print(f"Warning: Could not seed environment {rank}")
...
class VecRewardInfo(VecEnvWrapper):
    ...
    def reset(self):
        ...
        obs = self.venv.reset()
        print("Observations received from environment:", obs)
...
def make_env(env_id, seed, rank):
    def _thunk():
        ...
        # If the input has shape (H, W, C), handle typical image env
        obs_shape = env.observation_space.shape
        if len(obs_shape) == 3 and obs_shape[2] in [1, 3]:
            # If you want to handle Atari specifically, do your own wrappers here
            env = TransposeImage(env)
...
def make_vec_envs(env_name, seed, num_processes, num_frame_stack=None):
    ...
    print(f"Debug - Action space: {envs.action_space}, shape: {envs.action_space.n}")
    print(f"Debug - Observation space: {envs.observation_space}, shape: {envs.observation_space.shape}")
